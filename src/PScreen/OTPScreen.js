...
const OTPScreen = () => {
  ...
  const checkFullNameValidity = value => {
    const isValidLengths = /^.{4,40}$/;
    if (!isValidLengths.test(value)) {
      return 'Full Name must be atleast 4 Characters Long.';
    }
    const isContainsNumbers = /^(?=.*[0-9]).*$/;
    if (isContainsNumbers.test(value)) {
      return 'Full Name must not contain Digits.';
    }
    const isContainsSymbols =
      /^(?=.*[~`!@#$%^&*()--+={}\[\]|\\:;"'<>,.?/_₹]).*$/;
    if (isContainsSymbols.test(value)) {
      return 'Full name must not contain Special Symbols.';
    }
    return null;
  };
  ...
  const onRegister = () => {
    if (
      ...
      if (!checkPassowrd && !checkFullName) {
        setLoading(true);
        firebase
          .auth()
